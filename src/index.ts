...
// ---------------------------------------------------------------------------
// Rubric — translated from hiring-agent's resume_evaluation_criteria.jinja
// (HackerRank, MIT). This server applies the same scoring logic without
// requiring an external LLM. Useful for: portfolio audits, recruiter screens,
// candidate self-checks before applying.
// ---------------------------------------------------------------------------
...
function scoreSelfProjects(projects: NonNullable<ResumeJson["projects"]>): CategoryScore {
  if (projects.length === 0) {
    return { score: 0, max: 30, evidence: "No projects listed." };
  }
  ...
  for (const p of projects) {
    ...
    raw += pScore;
    evidence.push(
      `${p.name}: ${complexHits} complexity signals${hasLink ? ", link present" : ", NO LINK"}${isTutorial ? ", tutorial-flavoured" : ""} -> ${pScore}/10`
    );
  }
  ...
  return {
    score,
    max: 30,
    evidence: `Per-project breakdown: ${evidence.join(" | ")}. Total: ${score}/30.`,
  };
}

function scoreOpenSource(
  basics: ResumeJson["basics"],
  projects: NonNullable<ResumeJson["projects"]>
): CategoryScore {
  const githubLink = basics?.profiles?.find((p) =>
  ...
  if (!githubLink) {
    return {
      score: 0,
      max: 35,
      evidence: "No GitHub URL in resume header. Cannot assess open source contribution.",
    };
  }
  const externalSignals = projects.filter((p) => {
    const d = (p.description ?? "").toLowerCase();
    return (
      d.includes("contributed to") ||
      d.includes("pull request") ||
      d.includes("merged pr") ||
      d.includes("gsoc") ||
      d.includes("google summer of code")
    );
  });
  if (externalSignals.length === 0) {
    return {
      score: 6,
      max: 35,
      evidence: `GitHub URL present at ${githubLink.url}, but no external open source contributions detected in resume. Per rubric: all self_project -> capped at <=10. Light positive for having repos at all.`,
    };
  }
  ...
  return {
    score,
    max: 35,
    evidence: `Detected ${externalSignals.length} external contribution signal(s) in projects: ${externalSignals.map((p) => p.name).join(", ")}.`,
  };
}

function scoreProduction(work: NonNullable<ResumeJson["work"]>): CategoryScore {
  if (work.length === 0) {
    return { score: 0, max: 25, evidence: "No work history listed." };
  }
  ...
  const aiBoost = work.some((w) =>
    (w.highlights ?? []).some((h) => /openai|claude|gemini|llm/i.test(h))
  );
  if (aiBoost) score = Math.min(25, score + 2);
  return {
    score,
    max: 25,
    evidence: `~${years.toFixed(1)} years total production tenure across ${work.length} role(s)${aiBoost ? " (LLM production weighting +2)" : ""}.`,
  };
}
...
function scoreTechnicalSkills(resume: ResumeJson): CategoryScore {
  ...
  return {
    score,
    max: 10,
    evidence: `${breadth} distinct technologies/keywords detected across skills + projects.`,
  };
}
...
function bonusPoints(basics: ResumeJson["basics"]): { total: number; breakdown: string } {
  ...
  if (basics?.url) {
    total += 2;
    reasons.push(`+2 portfolio URL (${basics.url})`);
  }
  if (basics?.profiles?.some((p) => (p.network ?? "").toLowerCase().includes("linkedin"))) {
    total += 1;
    reasons.push("+1 LinkedIn profile");
  }
...
function buildEvaluation(resume: ResumeJson): Evaluation {
  ...
  const areas: string[] = [];
  if (scores.open_source.score < 12) {
    areas.push("Land 2-3 merged pull requests to popular open-source repos to break out of the <=10 self-only cap.");
  }
  if (ded.total > 0) {
    areas.push("Add live demo or repo URL to every project to remove missing-link deductions.");
  }
  if (scores.technical_skills.score < 9) {
    areas.push("List more concrete technologies/keywords to demonstrate stack breadth.");
  }
...
const server = new Server(
  { name: "resume-scorer-mcp", version: "0.1.0" },
  { capabilities: { tools: {} } }
);
  ...
  inputSchema: {
    ...
    properties: {
      resume_json: {
        type: "object",
        description:
          "Structured resume in JSON Resume format. Required keys: basics, work, projects, skills. See https://jsonresume.org/schema/",
      },
      resume_json_path: {
        type: "string",
        description: "Alternative to resume_json. Absolute path to a .json file on disk.",
      },
    },
  },
} as const;
...
const SCORE_FROM_TEXT_TOOL = {
  ...
  inputSchema: {
    type: "object",
    properties: {
      text: { type: "string", description: "Full resume text (plain text, no formatting)." },
    },
    required: ["text"],
  },
} as const;
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [SCORE_RESUME_TOOL, SCORE_FROM_TEXT_TOOL],
}));
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  if (name === "score_resume") {
    ...
    if (!resumeJson && path) {
      ...
      if (!existsSync(abs)) {
        return { content: [{ type: "text", text: `Error: file not found: ${abs}` }], isError: true };
      }
      resumeJson = JSON.parse(readFileSync(abs, "utf8"));
    }
    if (!resumeJson) {
      return {
        content: [{ type: "text", text: "Error: provide resume_json or resume_json_path." }],
        isError: true,
      };
    }
    ...
    if (!parsed.success) {
      return {
        content: [{ type: "text", text: `Error: invalid resume JSON: ${parsed.error.message}` }],
        isError: true,
      };
    }
  ...
  if (name === "score_resume_from_freeform") {
    ...
    const techs = Array.from(
      new Set(
        (text.match(/\b(Python|JavaScript|TypeScript|React|Next\.js|Node\.js|Java|Flutter|Firebase|OpenAI|Claude|Gemini|SQL|NoSQL|Selenium|Splunk|JIRA|Docker|Kubernetes|AWS|GCP|GraphQL)\b/gi) ?? [])
          .map((s) => s)
      )
    );
    ...
    const evaluation = buildEvaluation(resume);
    return {
      content: [
        {
          type: "text",
          text:
            "Best-effort freeform extraction. For accurate scoring use the score_resume tool with JSON Resume format.\n\n" +
